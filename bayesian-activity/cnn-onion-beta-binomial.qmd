...
![Frontpage of the Onion. Photo: Casey Bisson, via Flickr](../images/the_onion.jpg){fig-alt="Front page of Onion Newspaper: Headline reads Wikipedia Celebrates 750 Years of American Independence"}
...
## Run this set up code to load the functions from the `bayesrules` package first. There is no need to understand this code.
...
# Define a function to plot beta distributions with mean and mode
plot_beta <- function(alpha, beta) {
  df <- tibble(pi = seq(0, 1, length.out = 1000)) %>%
  ...
  if (alpha > 1 && beta > 1) {
    ...
    mode_val <- NULL  # set to NULL so it doesn't trigger ggplot error
  }

  p <- ggplot(df, aes(x = pi, y = density)) +
    geom_line(color = "black", linewidth = 1) +
    geom_vline(xintercept = mean_val, color = "blue", linetype = "solid", linewidth = 0.8) +
    labs(title = glue::glue("Beta({alpha}, {beta})"), x = expression(pi), y = "Density") +
    theme_minimal()

  if (!is.null(mode_val)) {
    p <- p + geom_vline(xintercept = mode_val, color = "blue", linetype = "dashed", linewidth = 0.8)
  }
...
plots <- map(betas, ~ plot_beta(.x[1], .x[2]))
...
my_prior <- plot_beta(alpha = my_alpha, beta = my_beta) +
...
3. Finally, we will compare to the full class data. This will require tallying all the scores from all students. 
...
summarize_beta_binomial(alpha = 8, beta = 3, y = 50, n = 150)
```

```{webr, fig.align = "center", fig.height = 5}
plot_beta_binomial(alpha = 8, beta = 3, y = 50, n = 150)
```
...
optimist <- plot_beta_binomial(8, 3, y = 50, n = 150) +
  labs(title = "Optimistic")
...
your_plot <- plot_beta_binomial(alpha = your_alpha, beta = your_beta, y = 50, n = 150) +
